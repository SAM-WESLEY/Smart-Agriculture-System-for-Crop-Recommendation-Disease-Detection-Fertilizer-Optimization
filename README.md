...
- ✅ Required fertilizers based on soil nutrient deficiencies
- ✅ Detecting plant diseases from leaf images using CNN
...
- 🧪 **Fertilizer Recommendation System** — suggests the right fertilizer based on soil nutrients and crop type
- 🦠 **Disease Detection using CNN** — identifies plant diseases from leaf images using a Convolutional Neural Network
- 🌐 **User-friendly Web Interface** — clean, responsive Flask web app accessible from any browser
...
```
[User Input / Leaf Image Upload]
         ↓
[Flask Web Application]
    ├── Crop Module       → Random Forest Classifier
    ├── Fertilizer Module → Random Forest Classifier
    └── Disease Module    → CNN (TensorFlow / Keras)
         ↓
[Prediction Result + Treatment Advice]
         ↓
...
| Input | Description |
|---|---|
| Leaf Image | JPG or PNG image of the plant leaf |
| Output | Disease name + treatment recommendation |
...
Apple Scab, Black Rot, Cedar Apple Rust, Bacterial Spot, Early Blight, Late Blight, Leaf Mold, Septoria Leaf Spot, Powdery Mildew — trained on the PlantVillage dataset.
...
├── disease_detection.py          # CNN disease detection logic
├── models/
